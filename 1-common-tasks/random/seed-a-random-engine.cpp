...
// we need a source of unpredictable seed data. On [6] we create a
// [`std::random_device`](cpp/numeric/random/random_device) for this purpose. On
// [7] we use a [`std::seed_seq`](cpp/numeric/random/seed_seq) to combine
...
// be. On [9] we create a random number engine using the `seed_seq` to seed the
