...
//
// Friendship in C++ is all-or-nothing; you cannot share only some
// methods with friendship. The [Passkey
// idiom](https://arne-mertz.de/2016/10/passkey-idiom/) works around that by
// "tagging" exposed methods with a `Key` parameter. If a caller
// class such as `baz` wants to call `foo`'s constructor or methods,
// it must be a friend of `Key`.
//
...
// member function `qux`. You simply pass a temporary object of type
