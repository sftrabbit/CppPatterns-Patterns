...
//
// The `Key` class defined on [4-7] does not need to be private or
...
//
// The `qux` function ([11-13]) is an example of a method that can
// only be called by `baz`. However, `foo`'s private methods (such as
// `bar()` on [16]) and variables stay private. This demonstrates how
// the Passkey idiom preserves encapsulation.
//
// [22-24] show how the caller would construct `foo` and call its
